...
The National Ecological Framework boundary maps come from a separate source
and are not included in the main package. Download them once, then read the
ecological level you need:
...
RSYC saves the downloaded file in its user data folder and checks that the file
is complete and unchanged. The file contains boundary maps only, not yield
curves. It is never downloaded when RSYC is installed or loaded. Organizations
that keep an approved copy elsewhere can provide its address through the
`RSYC.ecostrat_url` option.

The ecological framework was developed independently of RSYC and is published
by Agriculture and Agri-Food Canada. Contains information licensed under the
