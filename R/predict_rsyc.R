...
#' @param species A character string specifying the species code (e.g., `"PICE.MAR"` for black spruce). Use `species_codes()` for a full list of species names and codes.
#'
...
predict_rsyc <- function(tile_id, age, species) {
  ...
  #get params
  P <- RSYC::RSYC_params %>%
    dplyr::filter(TileID == tile_id, SpeciesCode == species)

  #check if params is one row
  if (nrow(P) == 0) {
    stop(glue::glue("No models for provided inputs"), call. = FALSE)
  }
  ...
  if (nrow(P) != 1) {
    stop(
      glue::glue(
        "Error! More than one model found. This cannot happen. Please submit a github issue"
      ),
      call. = FALSE
    )
  }
  ...
  if (any(age > 150, na.rm = TRUE)) {
    warning(
      "Stand age exceeds the model's calibration range (0–150 years). Predictions beyond this range involve extrapolation and may be unreliable.",
      call. = FALSE
    )
  }
